...
RSpec.describe "Certificates" do
  ...
  describe "GET /fo/registrations/:reg_identifier/certificate/pdf" do
        ...
        [false, 1, 0, :foo].each do |bad_value|
          context "when the value is '#{bad_value}'" do
            it "responds with a PDF" do
              get "/fo/registrations/#{registration.reg_identifier}/certificate/pdf?show_as_html=#{bad_value}"
              expect(response.content_type).to eq("application/pdf")
            end
          end
        end
    ...
    context "when the :block_front_end_logins feature toggle is active" do
      ...
      it "does not redirect to the application root" do
        get "/fo/registrations/#{registration.reg_identifier}/certificate/pdf"
